...
const isBehandlingAvTilbakekreving = (type: Kodeverk) => (type
  ? (type.kode === BehandlingType.TILBAKEKREVING || type.kode === BehandlingType.TILBAKEKREVING_REVURDERING) : false);

const accessibleFor = (validNavAnsattPredicates) => (navAnsatt: NavAnsatt) => validNavAnsattPredicates.some((predicate) => predicate(navAnsatt));

const enabledFor = (validFagsakStauses: string[], validBehandlingStatuses: string[]) => (
  fagsakStatus: Kodeverk, behandlingStatus: Kodeverk, isTilbakekrevingBehandling,
) => (
...
const accessSelector = (validNavAnsattPredicates, validFagsakStatuses, validBehandlingStatuses) => (navAnsatt: NavAnsatt,
  fagsakStatus: Kodeverk, behandlingStatus: Kodeverk, type: Kodeverk) => {
...
const godkjenningsFaneAccessSelector = (navAnsatt: NavAnsatt, ansvarligSaksbehandler: string) => {
...
const sendMeldingAccessSelector = (navAnsatt: NavAnsatt) => {
