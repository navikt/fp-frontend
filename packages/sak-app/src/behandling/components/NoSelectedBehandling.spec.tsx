...
describe('<NoSelectedBehandling>', () => {
  it('skal rendre korrekt melding', () => {
    ...
    expect(wrapper1.find('FormattedMessage').at(0).prop('id')).toEqual('NoSelectedBehandling.ZeroBehandlinger');

    const wrapper2 = shallow(<NoSelectedBehandling numBehandlinger={2} />);
    expect(wrapper2.find('FormattedMessage').at(0).prop('id')).toEqual('FagsakGrid.PleaseSelectBehandling');
  });
});
