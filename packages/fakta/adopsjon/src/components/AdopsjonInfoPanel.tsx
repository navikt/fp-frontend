...
export const AdopsjonInfoPanel = ({ isForeldrepengerFagsak, soknad, adopsjon }: Props) => {
  ...
  const ektefellesBarnAP = aksjonspunkterForPanel.find(ap => ap.definisjon === '5005');
  const mannAdoptererAleneAP = aksjonspunkterForPanel.find(ap => ap.definisjon === '5006');
      ...
      <RhfForm formMethods={formMethods} onSubmit={onSubmit} setDataOnUnmount={setMellomlagretFormData}>
        <VStack gap="space-24">
          <DokumentasjonFaktaForm
            ...
            hasEktefellesBarnAksjonspunkt={harAksjonspunkt('5005', aksjonspunkterForPanel)}
          />
...
const buildInitialValues = (adopsjon: AdopsjonFamilieHendelse, aksjonspunkterForPanel: Aksjonspunkt[]): FormValues => {
  const ektefellesBarnAP = harAksjonspunkt('5005', aksjonspunkterForPanel);
  const mannAdoptererAleneAP = harAksjonspunkt('5006', aksjonspunkterForPanel);
