      ...
      <RhfForm formMethods={formMethods} onSubmit={(values: FormValues) => submitCallback(transformValues(values))}>
        <VStack gap="space-20">
          ...
          <RhfRadioGroup
            ...
            size="small"
          >
            {Object.keys(omsorgsovertakelse.aktuelleDelvilkårAvslagsårsaker).map(kode => (
              <Radio size="small" key={kode} value={kode}>
          ...
          {delvilkår && vilkårUtfallType === 'IKKE_OPPFYLT' && (
            <RhfSelect
              ...
              selectValues={(omsorgsovertakelse.aktuelleDelvilkårAvslagsårsaker[delvilkår] ?? []).map(kode => (
                <option key={kode} value={kode}>
                  {alleKodeverk['Avslagsårsak'][map(delvilkår)].find(kodeverk => kodeverk.kode === kode)?.navn}
                </option>
              ))}
              readOnly={isReadOnly}
              validate={[required]}
...
const map = (omsorgovertakelsVilkår: OmsorgsovertakelseVilkårType): VilkårType => {
  switch (omsorgovertakelsVilkår) {
    case 'FP_VK_8F':
      return 'FP_VK_8';
    case 'FP_VK_16S':
      return 'FP_VK_16';
    default:
      return omsorgovertakelsVilkår;
  }
};
